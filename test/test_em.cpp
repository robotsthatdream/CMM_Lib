...
//        coord[1] = next_s(1)*MAX_Y;
        ...
        gmm.append_EM(Eigen::Vector2d((double)coord[0]/(double)MAX_X,(double)coord[1]/(double)MAX_Y),
              real_space[coord[0]][coord[1]]);

        if(real_space[coord[0]][coord[1]] == 0)
            nb_pos++;
        else if(real_space[coord[0]][coord[1]] == 1)
            nb_neg++;
        ...
        if(nb_pos >= gmm.max_component
                && nb_neg >= gmm.max_component){
            if(!is_init){
                gmm.EM_init();
                is_init = true;
            }else
